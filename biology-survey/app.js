...
const DATA = {
  ...
  subfields: [
    ...
    {
      key: "ab_synbio",
      name: "子领域2：抗体工程（41–60）",
      questions: [
        { id: 41, text: "Zero-shot 设计的边界： AI 能否完全不依赖同源序列模版，仅根据抗原的三维结构，从零生成具有纳摩尔级亲和力的全新抗体骨架？" },
        { id: 42, text: "胞内抗体的稳定性： 如何通过工程化改造，使抗体在还原性的细胞质环境中不仅保持热力学稳定，还能有效穿透细胞膜靶向胞内蛋白（如 RAS）？" },
        { id: 43, text: "多重逻辑门控： 如何构建基于“酸性 pH + 特异性蛋白酶 + 缺氧”多参数调控的分子逻辑开关，以彻底解决条件激活型抗体的“泄漏”问题？" },
        { id: 44, text: "非天然氨基酸的量产瓶颈： 如何克服正交翻译系统的效率障碍，实现含非天然氨基酸抗体在工业发酵罐中的高滴度、高保真生产？" },
        { id: 45, text: "高浓度的物理极限： 在浓度超过 200 mg/mL 时，究竟是哪种具体的分子间弱相互作用主导了抗体溶液的可逆聚集和粘度突增？" },
        { id: 46, text: "结合位点屏障效应： 对于实体瘤治疗，是否存在一个量化的亲和力（K_D）阈值，超过该值后抗体的组织渗透深度反而会因“结合即捕获”而显著下降？" },
        { id: 47, text: "可逆性自组装系统： 能否设计出一种在皮下注射制剂中形成低粘度团簇、而在进入血液后能迅速解离为单体的智能抗体系统？" },
        { id: 48, text: "正交组装的热力学机制： 在双特异性抗体设计中，除了空间位阻（Knobs-into-Holes），如何利用静电引导实现轻重链配对的绝对热力学正交？" },
        { id: 49, text: "六聚体的诱导组装： 如何通过 Fc 结构域的微小改造，使 IgG 仅在结合细胞表面抗原后才自发组装成六聚体，从而最大化补体杀伤效率？" },
        { id: 50, text: "体外生发中心的构建： 微流控芯片系统能否成功复现生发中心内的体细胞超突变和亲和力选择过程，实现抗体在体外的快速进化？" },
        { id: 51, text: "糖型的绝对均一化： 如何通过重塑宿主细胞的糖基转移酶网络，实现抗体药物糖链结构（如 G0F）的 100% 均一性控制？" },
        { id: 52, text: "复杂抗体的无细胞合成： 面对包含多个二硫键的复杂抗体，无细胞蛋白合成（CFPS）系统如何解决氧化折叠效率低和错误配对的难题？" },
        { id: 53, text: "连续进化的选择压力： 如何将噬菌体辅助连续进化（PACE）技术应用于抗体工程，实现亲和力与特异性的同步实时进化？" },
        { id: 54, text: "智能 Linker 的酶谱识别： 如何设计 ADC 连接子，使其能精准区分肿瘤微环境胞外酶与溶酶体酶的细微催化差异，以调控旁观者效应？" },
        { id: 55, text: "通用型去免疫化骨架： 是否存在一种通用的抗体蛋白支架，经过工程化改造后，在全球不同 HLA 亚型人群中均不诱发 T 细胞免疫反应？" },
        { id: 56, text: "激动剂的几何学控制： 针对 TNFR 等受体家族，抗体的表位距离和空间构型如何定量决定受体簇的组装效率和信号强度？" },
        { id: 57, text: "清道夫抗体的动力学极限： 为了实现单分子抗体清除 >1000 个抗原分子，抗体在内体酸性环境下的解离速率常数（k_off）需要达到什么量级？" },
        { id: 58, text: "隐蔽表位的捕获： 如何工程化抗体去识别并锁定蛋白质表面瞬态暴露、在静态结构中不可见的“隐蔽口袋”？" },
        { id: 59, text: "耐药突变的预测模型： 结合结构生物学与 AI，能否在临床试验前准确预测出病原体或肿瘤针对特定抗体最可能产生的逃逸突变路径？" },
        { id: 60, text: "多特异性的空间拓扑： 在构建三特异性抗体（如 T 细胞接合器 + 双检查点阻断）时，各结合域的空间距离应如何优化以形成最佳的免疫突触？" }
      ],
    },
    {
      key: "delivery",
    ...
    },
    {
      ...
      questions: [
        ...
        { id: 88, text: "抗原交叉呈递的速率匹配： 外源抗原从内体逃逸到胞质的速度常数，如何精准匹配 MHC-I 类分子在内质网中的肽段装载窗口期？" },
        ...
        { id: 99, text: "氧气梯度对效应功能的锁死： 深度缺氧环境如何通过调节 NK 细胞的离子泵活性，导致 ADCC 效应在颗粒释放阶段发生机械性中断？" },
